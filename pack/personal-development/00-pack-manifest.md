...
| 2026-09-08 | +1 map (MAP.002): карта 8 стержневых паттернов ЛР, композиция существующих методов/анти-паттернов/принципов/SoTA-заметок пака с ссылками на FPF-первоисточник (зафиксированный коммит); ничего нового не изобретено — только связка уже одобренных сущностей. Источник: пир-сессия Claude+Kimi+Codex (WP-565), поручение пилота. Total: 369→370 entities | Формализатор (Claude) |
...
| PD.CAT.002 | Leisure Practices | CAT | Каталог практик досуга и восстановления: 13 практик по 2 подпотокам (Энергия + Впечатления) с can-do, диагностикой засоров, привязкой к ступеням | draft |
| PD.CAT.003 | Practice Mastery Cards | CAT | Карточки 8 практик саморазвития × 4 степени мастерства: can-do, задание, assessment. Источник: Руков. 2 §2–§9 | draft |
...
| PD.CAT.006 | Applied Mastery Focus | CAT | Прикладные мастерства (D.194), которые пилот явно назвал предметом текущего развития - профессиональные, бытовые, ухода за собой. Декларируется пилотом, не выводится из поведенческих данных. | draft |
...
| PD.FORM.061 | Learner Qualification Ladder | FORM | Лестница зрелости в роли Ученика: 5 ступеней от Случайного (1) до Проактивного (5). Это внутренние ступени первой квалификационной роли, а не пять квалификаций; переходы подтверждаются двойным RCS-gate, а 4→5 — полным тройным gate. | active |
...
| PD.FORM.065 | Learner Time Budgets | FORM | Временные фазы наращивания бюджета ученика: W1-5, W6-25, W26+; не альтернативная лестница ступеней зрелости | active |
...
| PD.FORM.079 | Three Attention Objects Beginner | FORM | Три объекта внимания новичка: мировоззрение+цель, навыки+инструменты, ресурсы+окружение — агрегация 6 направлений и 9 характеристик для ступеней 1-2 | active |
...
| PD.FORM.081 | Five Development Areas | FORM | Проекция 6 направлений развития (FORM.002) в 5 тематических областей программы ЛР. Область отвечает на вопрос «что развивается» и не совпадает с контекстом практики из FORM.082 | draft |
...
| PD.FORM.086 | Semiosis As Metanoia Mechanism | FORM | Семиозис как механизм метанойи: переход между ступенями зрелости Ученика происходит через смену режима интерпретации знаков — не через накопление данных, а через переопределение смысла | active |
...
| PD.FORM.093 | Graduate Characteristics Catalog | FORM | Каталог 8 характеристик профиля Проактивного ученика: 5 обязательных и 3 supporting. FORM.093 задаёт две проверки полного gate ступени 5 — метрический профиль и свежее реальное изменение охвата ≥2; третью часть, двойной RCS-gate bh+cp, задаёт FORM.089. Ступень 5 может быть продвинутым завершением ЛР и даёт готовность к РР, но не означает фактический старт. | draft |
| PD.FORM.094 | Development Directions | FORM | Двулинейная модель развития Ученика: мировоззрение и мастерство с ненормативной балансировкой акцентов по ступеням. M1-M4 — учебные ветки, не поля текущего RCS; измерение использует 7 bh-характеристик и 13 cp-срезов FORM.089 | active |
| PD.FORM.095 | Artifact Role Accounting | FORM | Event Model артефактов созидателя: персональный репозиторий × тип артефакта × роль исполнителя. Артефакт превращается в событие Activity Hub и может питать разрешённые bh-характеристики FORM.089; cp-срезы определяет только Диагност по рубрикам и свидетельствам. Прямые веса W/M1–M4 и автоматическая запись cp запрещены. | active |
...
| PD.FORM.097 | Worldview As Executable Code | FORM | Мировоззрение как операционная модель: исполняемый код, ценности-инварианты, 4-слойная система и полный продуктовый режим 10ч+10$/нед | stable |
...
| PD.FORM.108 | Intellectual Rcs | FORM | Role Characterisation Space (RCS) для роли Интеллектуала — набор диагностических срезов мыслительного мастерства на степенях квалификации программы РР Работник → Стратег → Специалист. Аналог PD.FORM.089 (RCS Ученика), но для измерения зрелости мыслительной работы. Содержит 6 новых cp-срезов специфичных для Интеллектуала + переиспользует cp.wld, cp.int, cp.knw, cp.agt из FORM.089. Используется Диагностом, Портным, Аттестатором при ведении человека по программе РР. ВАЖНО: термин «степень квалификации», не «ступень» (D.164) | draft |
...
| PD.FORM.131 | Cyberhuman | FORM | Киберчеловек как модульная система «биологическое тело + экзотело»; для развития используется операционная проекция «биологическое тело ↔ IWE» | draft |
...
| PD.FORM.138 | Physical Layer Stage Zero | FORM | Защитное физическое pre-condition ст. 0: достаточная устойчивость организма для безопасного входа в ЛР; tri-state gate open/closed/indeterminate различает короткую просадку, устойчивый провал и неполные данные; это не ступень зрелости Ученика и не gate полного режима 10 ч/нед | draft |
| PD.FORM.139 | Mastery Matrix 2X2 | FORM | Матрица мастерства 2×2: (Субъект: Я / личная IWE) × (Режим: Управление / Развитие). Частный случай матрицы стиля жизни PD.FORM.060 для операционной проекции «биологическое тело ↔ IWE». | active |
...
| PD.MAP.002 | Personal Development Pattern Map | MAP | Восемь стержневых паттернов ЛР (проблема/силы → принцип → методы/анти-паттерны пака → FPF-первоисточник → сигнал); ссылки на FPF с зафиксированным коммитом, два паттерна помечены доменными оригиналами | active |
...
| PD.METHOD.015 | First Development Slot | METHOD | Первый слот саморазвития: конкретное действие + защищённое время + применённый след + рефлексия как точка входа в систему развития | active |
...
| PD.METHOD.030 | Шкала качества с якорями на каждом делении (Anchored Quality Scale) | METHOD | Метод конструирования шкалы самооценки с наблюдаемым якорем на каждом делении; локальная оценка ритуала не становится RCS-показателем без отдельной диагностики | active |
| PD.METHOD.031 | Node Tree + bh/cp views: структура модели развития созидателя | METHOD | Метод разделяет дерево развиваемых узлов, автоматически наблюдаемые bh-характеристики и диагностические cp-срезы; учебные ярлыки W/M1–M4 не подменяют поля RCS | draft |
| PD.METHOD.032 | Guide Section Template V4 | METHOD | 14-элементная структура подраздела персонального руководства с массивами frontmatter-проверок cp_checks и bh_checks | active |
...
| PD.METHOD.060 | Pre Registration Hypotheses Metrics Before Data Query | METHOD | — | draft |
...
| PD.ROLE.006 | Body As First System | ROLE | Роль мониторит базовую физическую устойчивость носителя ЛР, сигнализирует о просадке и открывает защитный переход ст. 0→1; ступени зрелости 1–5 подтверждаются отдельно | draft |
...
| PD.SOTA.006 | Adaptive Personalization | SOTA | Четыре ключевых алгоритма адаптивной персонализации (BKT, IRT, Spaced Repetition, Curriculum Learning) и их применение к программе ЛР: как SOTA влияет на каскад планирования (PD.METHOD.025), выбор контента Портным и метрики Диагноста | current |
...
| PD.WP.019 | Readme | WP | Заготовки для персонального руководства (personal-guide/{user}): 4 ступенных × 2 доменных вставки для bootstrap через /personal-guide-start | draft |
| PD.WP.019.domain-generic | Domain Generic | WP | Доменная вставка generic: fallback, когда проекты и Персона не дают явной профессиональной рамки; универсальные примеры и нейтральный язык | draft |
...
| PD.WP.019.qual-L7 | Qualification Reformer | WP | Заготовка руководства для степени квалификации Реформатор (PD.QUAL.001 L7): трансформация контекстов, создание новых подходов, влияние на сообщество. Ориентир — степень квалификации, не учебная ступень. | active |
| PD.WP.019.stage-1 | Stage 1 Random | WP | Заготовка руководства для ступени Случайного: безопасный личный проект, браузерный claude.ai и собственная заметка как первый фрагмент IWE с первого дня | draft |
...
> *Auto-generated by `generate-map.py` on 2026-08-25*
